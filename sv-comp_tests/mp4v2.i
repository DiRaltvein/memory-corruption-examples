...
uint8_t MP4GetVideoProfileLevel(uint8_t *buffer, size_t bufsize) {
  uint8_t *ptr = buffer;
  while (bufsize > 0) {
    if ((uint32_t)(((uint32_t(*(uint32_t *)ptr) & 0xFF000000U) >> 24) | ((uint32_t(*(uint32_t *)ptr) & 0x00FF0000U) >> 8) | ((uint32_t(*(uint32_t *)ptr) & 0x0000FF00U) << 8) | ((uint32_t(*(uint32_t *)ptr) & 0x000000FFU) << 24)) == 0x1b0) {
      return ptr[4];
    }
    ptr++;
    bufsize--;
