...
int json_tokener_parse_ex(json_tokener *tok, char *str, int len) {
  char c = '\1';
  tok->char_offset = 0;
  if ((len < -1) || (len == -1 && strlen(str) > (2147483647))) {
    return 1;
  }
  while ((((tok)->char_offset == len) ? 0 : (((c) = *str), 1))) {
    if (c == '\"') {
      if (!(++(str), ((tok)->char_offset)++, c) || !(((tok)->char_offset == len) ? 0 : (((c) = *str), 1))) {
        return 1;
      }
      char *strStart = str;
      while (c != '\"') {
      ...
      *str = '\0';
      ...
      int num = 0;
      while (c >= '0' && c <= '9' && num < 2147483647 / 10 - 10) {
        num = num * 10 + (c - '0');
        if (!(++(str), ((tok)->char_offset)++, c) || !(((tok)->char_offset == len) ? 0 : (((c) = *str), 1))) {
          return 1;
        }
...
int parseit(FILE *fd) {
  ...
  while ((ret = fread(buf, sizeof(char), sizeof(buf), fd)) > 0) {
    ...
    int start_pos = 0;
    while (start_pos != ret) {
      int error = json_tokener_parse_ex(&tok, &buf[start_pos], ret - start_pos);
      ...
      start_pos += parse_end;
      ((start_pos <= ret) ? (void) (0) : __assert_fail ("start_pos <= ret", "/mnt/a/master/master/sv-comp_tests/json-c.c", 99, __extension__ __PRETTY_FUNCTION__));
    }
  }
  return 0;
}
