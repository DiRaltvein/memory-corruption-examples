...
int gf_hevc_read_vps_bs_internal(HEVC_VPS *vps, GF_BitStream *bs) {
  uint8_t layer_id_included_flag[4][4];
  vps->max_layer_id = gf_bs_read_int(bs, 4);
  if (vps->max_layer_id > 4) {
    return -1;
  }
  vps->num_layer_sets = gf_bs_read_int(bs, 4);
  if (vps->num_layer_sets > 4) {
    return -1;
  }
  ...
  for (uint32_t i = 0; i < vps->num_layer_sets; i++) {
    for (uint32_t j = 0; j <= vps->max_layer_id; j++) {
      sum += layer_id_included_flag[i][j];
    }
  }
  return sum;
}
