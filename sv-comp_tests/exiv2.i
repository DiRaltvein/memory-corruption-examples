...
void tEXtToDataBuf(byte *bytes, uint32_t length) {
...
void printStructure(const char *filepath) {
  ...
  while (!eof(file, imgSize) && ::strcmp(chType, "IEND")) {
    ...
    buffer[dataOffset] = 0;
    fread(buffer, 1, dataOffset, file);
    uint32_t name_l = (uint32_t) std::strlen((const char*)buffer)+1;
    tEXtToDataBuf((byte*)buffer + name_l, dataOffset - name_l);
    fseek(file, 4, 1);
