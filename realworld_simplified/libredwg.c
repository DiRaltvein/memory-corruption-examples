// https://www.cvedetails.com/cve/CVE-2023-36273/
...
bit_calc_CRC(const uint16_t seed, unsigned char *addr, int len) {
  ...
  for (; len > 0; len--) {
    al = (unsigned char)((*addr) ^ ((unsigned char)(dx & 0xFF))); // Problem: in case addr and len do not match then buffer overread will happen
